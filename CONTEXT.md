...
- 作品编辑器·字幕改「对白框」（重定向）：去掉翻译字幕（底部小条）形态，播放文字统一用可移动对白框——对白=中间对白卡（角色名+对白内容自动加引号）始终显示，位置=三档预设（底/顶/中偏下）或播放中自由拖拽（存 x/y 百分比，`pos='custom'`），字号三档；场景=场景文字纯文字无框（内容=b.content，卡片正常显示），播放时可自由拖拽位置（x/y 百分比自动保存、写回真实数据、触屏可用）+字号三档（默认与对白一致），留空不显示；弹窗无独立字幕文字输入 → history.md「字幕改对白框」
