...
## 作品编辑器·卡牌RPG 分支（2026-08-24 完成，详见 docs/history.md）

- 用户拍板「底层相同、类型分支」：**不新开编辑器**，在现有作品编辑器里分支出卡牌RPG，共享 stories 存储 / 云同步 / 广场主页链路。
- 作品级 `kind`（story/card_rpg）+ `rpg:{hero,cards,enemies}` 存 data JSON（无 migration）；编辑器/播放器/展示三处分支。
- 卡牌RPG：侧边栏卡牌库/英雄/敌人配置；「⚔️ 卡牌战斗」积木（战前/胜败剧情 + 出战敌人多选）；播放走能量回合制战斗引擎（攻击/防御/治疗/回能/抽牌 5 种卡、敌人意图、护盾、胜败结算/重试）；未配卡兜底默认卡组。
- 展示：作品库/广场/个人主页按 kind 显示类型徽章（互动小说 / ⚔️ 卡牌RPG）。
- 待办：线上验收（创建卡牌RPG → 配卡配敌 → 战斗试玩 → 发布后广场徽章）。
...
- 卡牌RPG 一键生成示例 demo（2026-08-24）：作品编辑器新建区选「⚔️ 卡牌RPG」后显示「⚡ 一键生成卡牌RPG 示例」按钮——拉角色库选主角，自动生成完整可玩 demo（英雄/8 种 21 张卡/3 敌/单敌+多敌两场战斗）并直接打开，纯前端无后端改动 → history.md「卡牌RPG 一键生成示例 demo」
