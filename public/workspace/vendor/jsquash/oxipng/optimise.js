...
async function initMT(moduleOrPath) {
    const { default: init, initThreadPool, optimise, optimise_raw, } = await import('./pkg-parallel/squoosh_oxipng.js');
...
async function initST(moduleOrPath) {
    const { default: init, optimise, optimise_raw, } = await import('./pkg/squoosh_oxipng.js');
    await init(moduleOrPath);
    return { optimise, optimise_raw };
}
let wasmReady;
