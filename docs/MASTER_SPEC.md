...
**能做什么**：视觉小说 / 互动故事（**已可用** `make.html`）、互动视频（**下一档**，同款流程）、卡牌与 H5 游戏、漫画；**音频 BGM/配音**后续补；（音乐工具在 workspace）。

**两大核心**（用户心智，取代旧「四世界」入口）：

```
HYOOL
├── 创作作品 — 视觉小说(make) · 互动视频(筹备) · 漫画/卡牌/H5(专工)
...
- **视觉小说 / 互动故事**：`public/make.html` — 镜头列表 + 实时预览 + 试玩 + 发布
- 作品列表 / 删除 / 本地缓存同步修复；`story` 自动进 make，`?pro=1` 进专业编辑器

**下一档（同款 make 模式）**

- **互动视频**：`make-video.html`（筹备）— 镜头 + 视频轨 + 选项，复用 `stories` / `kind`
- **音频**：镜头 BGM、对白配音、章节曲 — 复用 `/api/upload` + 积木 `audio`/`bgm` 字段，make 壳内加面板
...
| **视觉小说（主）** | `public/make.html` |
| 互动视频（筹备） | `public/make-video.html` → 暂重定向或占位 |
