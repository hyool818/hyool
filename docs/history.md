...
## 作品编辑器产品愿景存档（2026-08-24，纯文档，无代码改动）

**需求**：用户提供 HYOOL 作品编辑器产品结构图（通用底层 × 5 类 19 种作品类型）。用户拍板：存档为产品愿景基线，先不动代码。

**改动**：
1. **`docs/editor-vision.md`（新建）**：完整产品树 + 解读（底层=引擎共用投资，类型=薄壳；Project State 应为类型无关文档模型）+ 现状对照（已实现/可复用/未实现）+ 硬约束（图片不上传服务器 → 素材库走引用 URL + 本地缓存 + D1 只存结构）。
2. **`CONTEXT.md`**：新增「作品编辑器产品愿景」小节（摘要 + 指向 editor-vision.md）。
