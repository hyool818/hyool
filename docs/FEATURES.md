...
| 小说生成 / 剧情提取 | `public/make.html` `public/make.js` | `POST /api/hub/novel-generate` `POST /api/hub/novel-extract` | `src/hub/novel.js`；有 `DEEPSEEK_API_KEY`（Secret / `.dev.vars`，不进 Git）则走 DeepSeek，否则 Workers AI |
