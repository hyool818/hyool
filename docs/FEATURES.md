...
| **主创作应用 · 视觉小说** | `public/make.html` `public/make.js` | 镜头 + 预览 + 试玩 + 发布；图片/视频镜；BGM/配音；选项**获得/需要物品**；**卡牌战镜头**（`rogue` → `story-rogue.js`，胜负写 `battle_win`） |
...
| 小说生成 / 剧情提取 | `public/make.html` `public/make.js` | `POST /api/hub/novel-generate` `POST /api/hub/novel-extract`（默认 `mode:compress` 选段压缩）`POST /api/hub/novel-prompt-rewrite` | `src/hub/novel.js`；选段→少量短旁白/对白；生图词中英；可追加当前作品 |
